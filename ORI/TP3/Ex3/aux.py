...
def normaliza (words, repeat = False, ord = True):
    ...
    palavras =  Regex.findall(r'\b[A-zÀ-úü]+\b', words.lower()) # ajuste pedido no tp3

    nltk.download('stopwords') # ajuste pedido no tp3
    ...
    for item in palavras:
        ...
        if elem != '':
            if elem not in bag and repeat == False and elem not in stopwords: # ajuste pedido no tp3
                bag.append(elem)
            elif repeat == True:
                bag.append(elem)
